...
static void _initializeLandmarksDiag(int k, const uint8_t* pMap, int mapWidth, int mapHeight);
...
int AStarFindPathLandmarksDiag(GridCoordinate src, GridCoordinate dest,
    ...
    int* outBuffer, int outBufferSize)
{
    _initializeLandmarksDiag(8, pMap, mapWidth, mapHeight);
    ...
    pq.push(make_tuple(heuristic(startNodeId), 0, startNodeId));
    while(!pq.empty())
    {
        int u = get<2>(pq.top()); pq.pop(); g_exploredNodes++;
        for (int e: directions)
...
void _initializeLandmarksDiag(int k, const uint8_t* pMap, int mapWidth, int mapHeight)
{
    LD.clear();
    g_landmarks.clear();
